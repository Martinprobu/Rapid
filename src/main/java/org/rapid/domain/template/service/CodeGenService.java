...
public class CodeGenService {
     ...
     * 列出表的字段描述
     */
    public List<TableDesc> descTable(String tableName, String path)  {
        ...
        JavaTableConverHandler handler = new JavaTableConverHandler();
        for (int i = 0; i < resultList.size(); i++) {
            map = resultList.get(i);
            table = handler.selectPlatForm(path).doHandler(map);//map.get("Key").toString(), map.get("Default").toString(), map.get("Extra").toString());
            list.add(table);
